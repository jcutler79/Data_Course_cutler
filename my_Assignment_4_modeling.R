...
glimpse(dat) # Glimpse is nicer than "str"
# A double ("<dbl>") is a unique format, it allows you to have 16 decimal spaces.
# Numeric stores 8 decimal spaces by default. Integers only take up 8 bits, doubles
# take up 32 bits.
# What data format would be best for transmission? Factor! It's two categories:
# automatic or manual
...
# quick plot
plot(dat) # whoa! Yeah it's *ucked up good.
...
x<-seq(-4,4,length=10)
y<-dnorm(x,mean=0, sd=1)
plot(x,y, type="l", lwd=2, main = "Normal Distribution 10")
...
# It's the variable that is affected by your explanatory variable

    # Which variables are explanatory?
# It's the variable(s) that affects your response variable. E.g. speed of golf swings is 
# something we would expect to be an explanatory variable of driving distances, driving
# distances being the response variable.
...
mod2 = glm(mpg ~ cyl*wt, data = dat) # With models you don't need quotes around 
# your column names. You should use the data = argument to save time and clean up!
tidy(mod2)
...
fake.data = abs(rnorm(100))
length(which(fake.data > 1))
length(which(fake.data < 1))
length(which(fake.data > 2))
...
######
rm(dat,dat.mod1,dat.mod2,decomp,decomp_model,M,mod1,mod2,mod3,model.cho.HDI,model.chol.HDI.meat)
rm(newdf,model.chol.HDIxmeat)
