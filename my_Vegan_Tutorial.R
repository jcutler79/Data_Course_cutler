...
# THE WAY THIS RARECURVE FUNCTION ESTIMATES DIVERSITY IS ACTUALLY PRETTY DEEP AND COOL. YOU
# TAKE OUT A RANDOM SAMPLE OF 100, AND IF YOU GET ONLY ONE OTU, THEN AT THE FIRST TAKE,
# YOUR SCORE IS ONE. IF ON THE FIRST STEP YOU GET 10 DIFFERENT OTU'S, THEN THAT IS YOUR
# SCORE. ON THE NEXT STEP, WITH THE NEXT TAKE OF A RANDOM SAMPLE OF 100, YOU COULD FIND MORE
# OTU'S THAT YOU DIDN'T FIND IN THE FIRST STEP. THUS, THE CURVE CAN CLIMB HIGHER ON THE Y
# AS YOU MOVE ALONG THE X.
...
beta_div = betadiver(community_matrix, method = "w") # read the paper cited in the help page for betadiver
beta_disp = betadisper(beta_div, treat)
plot(beta_disp)

library(scatterplot3d)
library(rgl) # needs xquartz
...
otu_table = read.delim("~/Desktop/Course_Materials/Data_Course/code_examples/Vegan_Example/vegan_example_otu_table.tsv", sep = "\t", row.names = 1)
metadata = read.delim("~/Desktop/Course_Materials/Data_Course/code_examples/Vegan_Example/vegan_example_metadata.tsv", sep = "\t", row.names = 1)
...
# WHOA, THIS LINE OF CODE ABOVE IS INTERESTING AND INCREDIBLY VALUABLE FOR ME (SEE MASTER LIST NOTEPAD, RBIND)
...
## WHY DO WE CARE ABOUT RAREFACTION IN OUR SAMPLING EFFORTS? BECAUSE WE'RE DOING ECOLOGY WITH NEXT GEN SEQUENCING AND WE
## HAVE NO IDEA WHY OR WHY NOT SOME SPECIES MIGHT BE OVER- OR UNDER-REPRESENTED.
...
### Check for marine and terrestrial
levels(metadata$Ecosystem)
...
# DISTANCE MATRICES ARE SUPER HANDY FOR DIVERSITY STUDIES, AND OTHER ECOLOGY STUDIES:
...
### If it says list of 35, that means it has 35 things I can access with the dollar sign. The one I want for plotting
### is called points:
NMDS = metaMDS(otu_table)
NMDS$points
### MDS = multi-dimensional scaling; NMDS = non-metric multi-dimensional scaling; No one uses the MDS without the N. It's all NMDS beotch!
### Non-metric doesn't require normally distributed data. It's robust against such non-normal data. HEY THAT'S PRETTY EFFING COOL!
### CHECK IT OUT:
MDS1 = NMDS$points[,1]; MDS1
MDS2 = NMDS$points[,2]; MDS2
# dev.off()
plot(MDS1,MDS2)
df = data.frame(MDS1 = MDS1, MDS2 = MDS2, Ecosystem = metadata$Ecosystem)
ggplot(df, aes(x = MDS1, y = MDS2, col = Ecosystem)) + geom_point() + 
  stat_ellipse()
...
### How do you do an ANOVA where the response variable is a matrix rather than a single vector??? THE WAY TO DO IT IS ADONIS!!!!!!!!
adonis(otu_table ~ metadata$Ecosystem) # THIS TELLS US THE PROBABILITY THAT THE MARINE AND TERRESTRIAL ECOSYSTEMS
# APPEAR DIFFERENT MERELY DUE TO CHANCE. IT TELLS US ONLY THAT AT LEAST ONE IS DIFFERENT FROM ANOTHER ONE. IF THERE
# ARE TWENTY THINGS TO COMPARE, IT DOESN'T TELL YOU WHICH. 
### THE STATISTICAL CALCULATIONS BEHIND THIS PERMUTATIONAL ANOVA SOFTWARE MUST BE INSANE. WE DON'T HAVE JUST ONE RESPONSE VARIABLE. WE HAVE LIKE 39!!!

### Now try interaction model:
adonis(otu_table ~ metadata$Ecosystem*metadata$Host)
df = data.frame(MDS1 = MDS1, MDS2 = MDS2, Ecosystem = metadata$Ecosystem, Host = metadata$Host)

ggplot(df, aes(x = MDS1, y = MDS2, col = Host)) + geom_point() + 
  stat_ellipse()

## Numerical Ecology, the book, written by one of the dudes who developed permanova. Legendre is also
# one of the main guys who developed permanova adonis stuff, in 1999.

## manova is multiple analysis of variance for multiple INDEPENDENT response variables. permanova is for multiple DEPENDENT response variables.
