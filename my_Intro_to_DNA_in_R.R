...
dir.create("./filtered/")
for (i in filt.files){
  fastqFilter(fn = i,fout = paste0(getwd(),"/filtered/",i,".filt"), truncLen = 150)
}
# Don't forget to get rid of all the primers when you filter. Filtering is VERY important. Fastx or dada2 can do that
?fastqFilter
setwd(file.path(paste0(getwd(),"/filtered")))
dir()
filtered = dir(path = getwd(), pattern = ".filt"); filtered
dr = derepFastq(filtered); dr # DEREPLICATE IS IMPORTANT!
err = learnErrors(dr) # ERROR IS TELLING US ABOUT THE QUALITY, MISREADINGS
plotErrors(err)
clean = dada(dr, err)
...
library(devtools)
install_github("kingaa/pomp") # DOESN'T WORK. SAYS DIRECTORY NOT FOUND FOR ..., AND LIBRARY NOT FOUDNF RO -LGFORTRAN
